...
export async function POST(req: NextRequest) {
  ...
  const { ticker, type, shares: sharesInput, amountEur, price_override, company_name } = await req.json()
  ...
  // Si viene price_override (crypto), usarlo directamente sin llamar a Polygon
  let price: number
  let displayName: string

  if (price_override && price_override > 0) {
    price = parseFloat(price_override)
    displayName = company_name ?? ticker.toUpperCase()
  } else {
    // Obtener precio real de Polygon (acciones)
    try {
      const quote = await getSnapshot(ticker.toUpperCase())
      price = quote.price
      displayName = quote.ticker
    } catch {
    ...
    if (type === 'buy') {
      ...
      if (existing) {
        ...
        await client.query(
          ...
          [portfolio.id, ticker.toUpperCase(), displayName, shares, price, price]
        )
      }
      ...
      await client.query(
        ...
        [portfolio.id, ticker.toUpperCase(), displayName, 'buy', shares, price, totalAmount, cashBefore, cashAfter]
      )
      ...
      if (!position || parseFloat(position.shares) < shares) {
        await client.query('ROLLBACK')
        return NextResponse.json({
          error: `No tienes suficientes de ${ticker}`
        }, { status: 400 })
      }
      ...
      await client.query(
        'INSERT INTO transactions (portfolio_id, ticker, company_name, type, shares, price_per_share, total_amount, cash_before, cash_after) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
        [portfolio.id, ticker.toUpperCase(), displayName, 'sell', shares, price, totalAmount, cashBefore, cashAfter]
      )
